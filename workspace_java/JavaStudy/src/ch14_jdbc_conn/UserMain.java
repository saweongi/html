...
public class UserMain {
	public static void main(String[] args) {
		...
		UserService bbsService = UserService.getInstance();
		while(true) {
			 ...
			 if(command == 1) {
				 ...
				 if(pw.equals(userVO.getUserPw())) {
					 ...
					 for(int i = 0 ; i < list.size(); i ++) {
						 String result =  MessageFormat.format("게시글 {0} | {1} | {2} |"
								 , list.get(i).getRnum(), 
					 ...
					 if(command == 1) {
						 ...
						 //게시글 중 1입력시 게시글 1의 상세 내용과 댓글을 출력하세요
						 //(1) 상세내용을 담을 VO 생성 or 기존 VO 컬럼추가
						 //(2) 상세내용 query작성
						 //(3) dao, service 메서드 추가
						 //(4) 1입력시 1에 해당하는 bbsno로 조회내용을 출력
						 command = Integer.parseInt(sc.nextLine());
						 ArrayList<BbsVO> list1 = userService.bbsList();
						 for(int i = 0 ; i < list1.size(); i ++) {
							 String result =  MessageFormat.format("게시글 {0} | {1} | {2} |{3} | {4}|"
									 , list1.get(i).getRnum(),list1.get(i).getBbsContent(), 
									 list1.get(i).getBbsContent(), list1.get(i).getAuthorId()
									 ,list1.get(i).getUpdateDt());
							 System.out.println(result);
						 }
					 }
				 }else {
