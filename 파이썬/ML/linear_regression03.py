...
ax = plt.axes(projection = '3d') #축이 한개 더생겨서(x2)  3차원으로 나타냄  --> x가 계속 늘어 날수록 차원이 높아진다
ax.set_xlabel('study')
...
x1_data = np.array(x1) #넘파이 배열 만들기
x2_data = np.array(x2) #넘파이 배열 만들기
y_data = np.array(y)
a1 = 0   #기울기 1
a2 = 0   #기울기 2
b = 0   #y절편
...
for i in range(epochs):
    y_hat = a1 * x1_data +a2 * x2_data + b #1차 방정식 (y=ax +b)
    ...
    if i % 100 ==0:
        print("epochs=%.f, 기울기1 a1=%.04f" ",기울기2 a2=%.04f, y절편=%.04f" %(i, a1,a2, b))

##3차원 그리는거 근데 넘 복잡해서 에바
