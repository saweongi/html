...
# 배열과 차이 (print해서 보면 , 가없으면 넘파일자료형이다
arr = [1, 2, 3, 4]
print("arr:",arr)
...
#dot product ->벡터의 곱 할때 사용
data3 = np.dot(data1, data2)
print(data3)

#전치행렬(대각으로 숫자를바꿔주는 행렬)
data4 = np.transpose(data3)
print(data4)

test = np.arange(10) #0~9까지 1차원으로 만들어줌
print(test)
print(test.shape, test.dtype, test.ndim)

#형태변경 reshape ->차원을 바꿀때사용
print('2 x 5', test.reshape(2,5))
print('5 x 2', test.reshape(5,2))
print('1d', test.reshape(-1,1)) #행으로되어있는걸 열로바꿈
print('1', test.reshape(-1)) #열을 행으로 바꿈